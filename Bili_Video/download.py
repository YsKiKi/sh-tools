...
def get_bangumi_video_info(ep_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        for section in [result.get('episodes', []), *[s.get('episodes', []) for s in result.get('section', [])]]:
            if not section:
                continue
            target_ep = next((ep for ep in section if str(
                ep.get('id', '')) == ep_id or str(ep.get('ep_id', '')) == ep_id), None)
            if target_ep:
                break

        if not target_ep and result.get('main_section', {}).get('episodes'):
            target_ep = next((ep for ep in result['main_section']['episodes'] if str(
                ep.get('id', '')) == ep_id or str(ep.get('ep_id', '')) == ep_id), None)

        if not target_ep:
            target_ep = result.get('episodes', [{}])[
                0] if result.get('episodes') else None
...
def get_bili_video_with_session(bvid: str, cid: str, sessdata: str, qn: int, smart_resolution: bool = False) -> Dict:
    ...
    api_url = BILI_PLAY_STREAM.replace("{bvid}", bvid).replace("{cid}", cid).replace(
        "{qn}", str(qn)).replace("{fnval}", str(fnval)).replace("{fourk}", str(fourk))
...
def get_bangumi_bili_video_with_session(ep_id: str, cid: str, sessdata: str, qn: int, smart_resolution: bool = False) -> Dict:
    ...
    api_url = BILI_BANGUMI_STREAM.replace("{ep_id}", ep_id).replace("{cid}", cid).replace(
        "{qn}", str(qn)).replace("{fnval}", str(fnval)).replace("{fourk}", str(fourk))
    ...
    if json_data.get('code') != 0:
        raise Exception(
            f"番剧API错误: {json_data.get('message', '未知错误')} (code: {json_data.get('code')})")
...
def select_and_avoid_mcdn_url(base_url: str, backup_urls: List[str] = None) -> str:
    ...
    slow_cdn_patterns = ['.mcdn.bilivideo.cn',
                         'mountaintoys.cn', '.szbdyd.com']
...
def get_download_url(url: str, sessdata: str, qn: int, duration: int = 0, smart_resolution: bool = False, file_size_limit: int = 100, preferred_codec: str = 'auto') -> Dict[str, Optional[str]]:
    ...
    _log(
        f"[BILI下载] 开始获取视频下载链接，视频ID: {video_id}, 请求画质: {quality_text}, QN: {qn}, 编码选择: {preferred_codec}")
    ...
    if is_bangumi:
        # 番剧使用专门的API
        bangumi_result = get_bangumi_bili_video_with_session(
            ep_id, cid, sessdata, qn, smart_resolution)
        ...
        if stream_type == 'durl':
            ...
            video_url = select_and_avoid_mcdn_url(
                first_durl['url'], backup_urls)
            _log(
                f"[BILI下载] 番剧DURL格式，视频大小: {round(first_durl.get('size', 0) / 1024 / 1024)}MB, 时长: {round(first_durl.get('length', 0) / 1000)}秒")
            from urllib.parse import urlparse
            _log(f"[BILI下载] 选中的下载URL: {urlparse(video_url).hostname}")
            return {'videoUrl': video_url, 'audioUrl': None}
    else:
        # 普通视频
        stream_data = get_bili_video_with_session(
            video_id, cid, sessdata, qn, smart_resolution)

    # 以下是DASH格式处理逻辑
    dash_data = stream_data.get(
        'data', stream_data) if is_bangumi else stream_data
    ...
    if target_height is None:
        # 未知QN，使用最高画质
        target_height = max([v.get('height', 0)
                            for v in video]) if video else 1080
    ...
    if smart_resolution:
        ...
        available_heights = sorted(
            set([v.get('height', 0) for v in video]), reverse=True)
        _log(
            f"[BILI下载] 智能分辨率模式：使用所有可用画质 {', '.join([str(h) + 'p' for h in available_heights])}")
    else:
        matching_videos = [v for v in video if v.get(
        ...
        if not matching_videos:
            available_heights = sorted(
                set([v.get('height', 0) for v in video]), reverse=True)
            _log(
                f"[BILI下载] ⚠️ 请求的{target_height}p画质不可用，API返回的最高画质: {available_heights[0]}p")
            _log(
                f"[BILI下载] API可用分辨率列表: {', '.join([str(h) + 'p' for h in available_heights])}")
            ...
            if matching_videos:
                ...
                matching_videos = [
                    v for v in matching_videos if v.get('height') == max_height]
                _log(f"[BILI下载] 降级使用: {max_height}p")
        else:
            _log(f"[BILI下载] ✅ 找到匹配的{target_height}p画质")

        if not matching_videos:
            min_height = min([v.get('height', 0)
                             for v in video]) if video else 0
            matching_videos = [
                v for v in video if v.get('height') == min_height]
    ...
    if matching_videos:
        ...
        def estimate_size(stream: Dict, audio_stream: Optional[Dict], timelength: int) -> float:
            ...
            audio_bandwidth = audio_stream.get(
                'bandwidth', 0) if audio_stream else 0
            total_bandwidth = video_bandwidth + audio_bandwidth
            bytes_per_second = total_bandwidth / 8
            duration_seconds = timelength / 1000 if timelength > 0 else (stream.get(
                'duration', 0) or (audio_stream.get('duration', 0) if audio_stream else 0))
        ...
        if timelength == 0 and not is_bangumi:
            ...
            max_total_bandwidth = (
                file_size_limit * 1024 * 1024 * 8) / assumed_duration
            _log(
                f"[BILI下载] 假设视频时长{assumed_duration}秒，计算最大总码率: {round(max_total_bandwidth / 1024)}kbps")

            for candidate in sorted_videos:
                video_bandwidth = candidate.get('bandwidth', 0)
                audio_bandwidth = audio_data.get(
                    'bandwidth', 0) if audio_data else 0
                total_bandwidth = video_bandwidth + audio_bandwidth
                ...
                if total_bandwidth <= max_total_bandwidth:
                    ...
                    _log(
                        f"[BILI下载] 选择视频流: {candidate.get('height')}p, 编码: {codec_type.upper()}, 总码率: {round(total_bandwidth / 1024)}kbps")
                    break
            ...
            if smart_resolution:
                available_heights = sorted(
                    set([v.get('height', 0) for v in video]), reverse=True)
                _log(
                    f"[BILI下载] 智能分辨率：从最高{available_heights[0]}p开始，符合{file_size_limit}MB限制")

                for height in available_heights:
                    height_videos = [
                        v for v in video if v.get('height') == height]
                    sorted_height_videos = sorted(height_videos, key=lambda v: (
                        codec_priority.get(get_codec_type(
                            v.get('codecs', '')), 999),
                    ...
                    for candidate in sorted_height_videos:
                        estimated_size = estimate_size(
                            candidate, audio_data, timelength)
                        if estimated_size <= file_size_limit:
                            video_data = candidate
                            codec_type = get_codec_type(
                                candidate.get('codecs', ''))
                            _log(
                                f"[BILI下载] ✅ 智能分辨率选择: {candidate.get('height')}p, 编码: {codec_type.upper()}, 预估大小: {round(estimated_size)}MB")
                            break
                ...
                if not video_data:
                    ...
                    lowest_videos = [v for v in video if v.get(
                        'height') == lowest_height]
                    video_data = sorted(
                        lowest_videos, key=lambda x: x.get('bandwidth', 0))[0]
            else:
                # 非智能分辨率
                if is_bangumi:
                    # 番剧特殊处理
                    bangumi_videos = [v for v in video if v.get(
                        'height') == target_height]
                    if not bangumi_videos:
                        max_height = max([v.get('height', 0) for v in video])
                        bangumi_videos = [
                            v for v in video if v.get('height') == max_height]
                    ...
                    for candidate in sorted_videos:
                        ...
                        if estimated_size <= file_size_limit:
                            ...
                            codec_type = get_codec_type(
                                candidate.get('codecs', ''))
                            _log(
                                f"[BILI下载] 选择视频流: {candidate.get('height')}p, 编码: {codec_type.upper()}, 预估大小: {round(estimated_size)}MB")
                            break
    ...
    if audio:
        ...
        audio_url = select_and_avoid_mcdn_url(
            audio_base_url, audio_backup_urls)
...
def get_resolution_labels(selected_value: int) -> str:
    ...
    filtered_resolutions = [
        r for r in BILI_RESOLUTION_LIST if r['value'] >= selected_value]
