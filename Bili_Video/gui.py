...
def get_special_folder_path(folder_id):
    ...
    if sys.platform == 'win32':
        ...
        ctypes.windll.shell32.SHGetFolderPathW(
            None, folder_id, None, SHGFP_TYPE_CURRENT, buf)
...
class M4SProcessorGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.url_input.setPlaceholderText(
            "支持: BV号、AV号、视频链接、番剧EP号(ep123456)、番剧链接")
    ...
    def setup_connections(self):
        ...
        self.input_path_edit.editingFinished.connect(
            self.save_input_path_on_edit)
        self.output_path_edit.editingFinished.connect(
            self.save_output_path_on_edit)
    ...
    def show_config_info(self):
        ...
        self.log_message(
            f"输入路径: {self.config_manager.config.get('input_path', '未设置')}")
        self.log_message(
            f"输出路径: {self.config_manager.config.get('output_path', '未设置')}")

        # 更新界面显示
        self.input_path_edit.setText(
            self.config_manager.config.get("input_path", ""))
        self.output_path_edit.setText(
            self.config_manager.config.get("output_path", ""))
        # 恢复画质选择
    ...
    def show_qr_login(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if sessdata:
                self.config_manager.save_session(
                    sessdata=sessdata, refresh_token=refresh_token)
    ...
    def start_download(self):
        ...
        if ep_match:
            ...
            if not url.startswith('http'):
                url = f"https://www.bilibili.com{url}" if url.startswith(
                    '/') else f"https://www.bilibili.com/bangumi/play/ep{ep_id}"
        ...
        try:
            ...
            if is_bangumi:
                ...
                self.log_message(
                    f"集数: {ep_info.get('ep_title', f'EP{ep_id}')}")
            else:
                # 普通视频信息获取
            ...
            audio_file = temp_dir / \
                f"{safe_title}_audio.m4s" if audio_url else None
            ...
            download.download_b_file(
                video_url, str(video_file), progress_callback)
            # 清除进度行，显示完成消息
            ...
            if audio_url:
                ...
                self._progress_line_exists = False
                download.download_b_file(
                    audio_url, str(audio_file), progress_callback)
                if self._progress_line_exists:
                ...
                if video_ext == '.mp4':
                    ...
                    try:
                        ...
                        ffmpeg.run(ffmpeg.overwrite_output(output_stream), quiet=True,
                                   overwrite_output=True, creationflags=subprocess.CREATE_NO_WINDOW)
        ...
        try:
            ...
            if result.get('SESSDATA'):
                self.login_success.emit(
                    result['SESSDATA'], result.get('refresh_token', ''))
            else:
...
# def load_fonts():
#     wenkai_path = resource_path("res/LXGWWenKai-Medium.ttf")
#     wenkai_id = QFontDatabase.addApplicationFont(str(wenkai_path))
#     wenkai_family = QFontDatabase.applicationFontFamilies(wenkai_id)[0]

#     return wenkai_family


# def make_app_font(chinese_font: str, size=10) -> QFont:
#     font = QFont()
#     font.setPointSize(size)
#     font.setFamilies([chinese_font])
#     return font
...
def main():
    ...
    # chinese_font = load_fonts()
    # app_font = make_app_font(
